...
public sealed class ActorRegistry
{
    ...
    public void UnregisterSpeech(string actorId, ISpeechService s)
    {
        if (string.IsNullOrWhiteSpace(actorId)) return;
        if (_speech.TryGetValue(actorId, out var cur) && ReferenceEquals(cur, s)) _speech.Remove(actorId);
    }
    public void UnregisterBot(string actorId, IBotController b)
    {
        if (string.IsNullOrWhiteSpace(actorId)) return;
        if (_bots.TryGetValue(actorId, out var cur) && ReferenceEquals(cur, b)) _bots.Remove(actorId);
    }
    public void UnregisterRootTransform(string actorId, Transform t)
    {
        if (string.IsNullOrWhiteSpace(actorId)) return;
        if (_roots.TryGetValue(actorId, out var cur) && ReferenceEquals(cur, t)) _roots.Remove(actorId);
    }
